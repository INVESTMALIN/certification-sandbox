...
import { OCCUPATION_DATA } from './occupationData'

// ── Graphique linéaire (axes flexibles) ───────────────────────────────────────
function OccupationChart({ current, previous, yMax, yUnit }) {
    const w = 460, h = 170
    const pL = 52, pB = 28, pT = 8, pR = 10
    const iW = w - pL - pR
    const iH = h - pT - pB
    ...
    const formatLabel = (val, idx) => {
        ...
        if (yUnit === '%') {
            return isEdge ? `${val.toFixed(1)}%` : `${val}%`
        }
        if (yUnit === 'j') {
            const rounded = parseFloat(val.toFixed(2))
            return isEdge ? `${rounded} jours` : `${rounded}`
        }
        // €
        return `${val}€`
    }
        ...
        <svg viewBox={`0 0 ${w} ${h}`} className="w-full">
            {yVals.map((val, idx) => (
                <g key={idx}>
                    ...
                    <text x={pL - 5} y={getY(val) + 4} textAnchor="end" fontSize="9.5" fill="#9ca3af">
                        {formatLabel(val, idx)}
                    </text>
                </g>
            ))}
...
// ── Page générique Occupation et tarifs ───────────────────────────────────────
export default function OccupationPage() {
    const { slug } = useParams()
    const data = OCCUPATION_DATA[slug]

    if (!data) return <Navigate to="/airbnb/performance/occupation/taux-occupation" replace />

    return <OccupationContent data={data} />
}

function OccupationContent({ data }) {
                    ...
                    <div className="px-8 py-6 max-w-5xl">
                        ...
                        <div className={`flex mb-8 ${data.stats.length > 1 ? 'gap-0 divide-x divide-gray-200' : ''}`}>
                            {data.stats.map((stat, i) => (
                                <div key={i} className={data.stats.length > 1 ? 'px-6 first:pl-0' : ''}>
                                    <div className="text-3xl font-bold text-gray-900 mb-1">{stat.value}</div>
                                    <a href="#" className="text-sm text-gray-600 underline">{stat.label}</a>
                                </div>
                            ))}
                        </div>
